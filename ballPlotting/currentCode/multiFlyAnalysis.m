...
numTrialsToUse = prefixCodeTrialNums(prefixCode);
...
for fly = 1:size(flies,1)
    ...
    for stimNum = unique(groupedData.stimNum)
        allFastTrials = find(groupedData.trialSpeed > threshold & groupedData.stimNum == stimNum);
        selectFastTrials = allFastTrials(1:numTrialsToUse);
        
        % Get the mean of those trials  
        if fly == 1
            allFliesXDisp{stimNum} = groupedData.rotXDisp(selectFastTrials,:);
            allFliesYDisp{stimNum} = groupedData.rotYDisp(selectFastTrials,:);
        else
            allFliesXDisp{stimNum} = [allFliesXDisp{stimNum};groupedData.rotXDisp(selectFastTrials,:)];
            allFliesYDisp{stimNum} = [allFliesYDisp{stimNum};groupedData.rotYDisp(selectFastTrials,:)];
        end
...
%%
for stimNum = unique(groupedData.stimNum)
    plotData.meanXDisp(stimNum,:) = mean(allFliesXDisp{stimNum});
    plotData.meanYDisp(stimNum,:) = mean(allFliesYDisp{stimNum});
end

figure; plot(plotData.meanXDisp',plotData.meanYDisp')
xlim([-3 3])
