function plotBallDataAllStimSameFig(prefixCode,expNum,flyNum,flyExpNum,allTrials)
...
for i = 1:length(uniqueStim)
    ...
    t{i} = ['Successful ',num2str(StimStruct(i).stimObj.speakerAngle),' trials = ',num2str(length(stimNumInd)),'/',num2str(length(stimNumIndNotSelected))];
    if i == 3
        title([char(t{1}),'   ',char(t{2}),'  ',char(t{3})])
    end
    ...
    ylabel('Y displacement (mm)')
    legendText{i} = ['Speaker Angle = ',num2str(StimStruct(i).stimObj.speakerAngle)];
